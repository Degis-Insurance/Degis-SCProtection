...
interface IProposalCenter {
    ...
    function proposalIds(uint256) view external returns (string memory protocolName, address protocolAddress, address proposerAddress, uint256 maxCapacity, uint256 initialpolicyPricePerShield, uint256 timestamp, uint256 yes, uint256 no, uint256 round, bool pending, bool approved);
    function proposePool(address _protocol, string memory _name, uint256 _maxCapacity, uint256 _initialpolicyPricePerShield) external;
