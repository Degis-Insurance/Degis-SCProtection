...
import "src/reward/farming/WeightedFarmingPoolEventError.sol";
    ...
    ContractSetupBaseTest
{
    ...
    function testRemoveLiquidity() public {
        ...
        vm.prank(CHARLIE);
        vm.expectRevert(ProtectionPool__NotEnoughLiquidity.selector);
        ...
        console.log("ALICE BALANCE:", protectionPool.balanceOf(ALICE));
        ...
        // # --------------------------------------------------------------------//
        uint256 balance = protectionPool.balanceOf(CHARLIE);
        vm.prank(CHARLIE);
        vm.expectEmit(false, false, false, true);
        emit LiquidityRemoved(CHARLIE, balance);
        policyCenter.removeLiquidity(balance);
    ...
    function testStake() public {
        ...
        vm.prank(CHARLIE);
        vm.expectEmit(false, false, false, true);
        emit LiquidityStaked(CHARLIE, JOE_ID, LIQUIDITY * 9999 / 10000);
        policyCenter.stakeLiquidity(JOE_ID, LIQUIDITY * 9999 / 10000);
        ...
        vm.expectRevert("ERC20: transfer amount exceeds balance");
        emit LiquidityStaked(CHARLIE, JOE_ID, LIQUIDITY * 9999 / 10000);
        policyCenter.stakeLiquidity(PTP_ID, LIQUIDITY * 9999 / 10000);
        ...
        vm.prank(CHARLIE);
        vm.expectEmit(false, false, false, true);
        emit LiquidityStaked(CHARLIE, JOE_ID, LIQUIDITY / 2 * 9999 / 10000);
        policyCenter.stakeLiquidity(JOE_ID, LIQUIDITY / 2 * 9999 / 10000);

        vm.prank(CHARLIE);
        vm.expectEmit(false, false, false, true);
        emit LiquidityStaked(CHARLIE, PTP_ID, LIQUIDITY / 2 * 9999 / 10000);
        policyCenter.stakeLiquidity(PTP_ID, LIQUIDITY / 2 * 9999 / 10000);
        ...
        vm.prank(CHARLIE);
        vm.expectEmit(false, false, false, true);
        emit LiquidityStaked(CHARLIE, PTP_ID, LIQUIDITY * 9999 / 10000);
        policyCenter.stakeLiquidity(PTP_ID, LIQUIDITY * 9999 / 10000);
    ...
    function _stake(address _user) private {
        ...
        uint256 balance = protectionPool.balanceOf(_user);
        vm.prank(_user);
        policyCenter.stakeLiquidity(JOE_ID, balance);
    }
    ...
    function testUnstake() public {
        ...
        uint256 price = protectionPool.getLatestPrice();
        _provideLiquidity(ALICE);
        console.log("Price:", price);
        console.log("ALICE PRO POOL BAL:", protectionPool.balanceOf(ALICE));
        console.log("ALICE LQIQUIDITY S:", LIQUIDITY);
        console.log("PRO POOL SHIELD BAL", shield.balanceOf(address(protectionPool)));
        vm.prank(ALICE);
        protectionPool.approve(address(policyCenter), LIQUIDITY * SCALE / price);
        vm.prank(ALICE);
        policyCenter.stakeLiquidity(PTP_ID, LIQUIDITY * SCALE / price);

        // # --------------------------------------------------------------------//
        // # Should not be able to unstake liquidity without prior staking # //
        // # --------------------------------------------------------------------//

        vm.prank(CHARLIE);
        vm.expectRevert(WeightedFarmingPool__NotEnoughAmount.selector);
        ...
        emit LiquidityUnstaked(CHARLIE, JOE_ID, joeLPAddress, LIQUIDITY / 2);
        policyCenter.unstakeLiquidity(JOE_ID, joeLPAddress, LIQUIDITY / 2);
        ...
        vm.prank(CHARLIE);
        vm.expectEmit(true, true, false, true);
        emit LiquidityUnstaked(CHARLIE, JOE_ID, joeLPAddress, LIQUIDITY * 9999 / 10000);
        policyCenter.unstakeLiquidity(JOE_ID, joeLPAddress, LIQUIDITY * 9999 / 10000);
        ...
        vm.prank(ALICE);
        vm.expectEmit(true, true, false, true);
        emit LiquidityUnstaked(ALICE, PTP_ID, ptpLPAddress, LIQUIDITY * 9999 / 10000);
        policyCenter.unstakeLiquidity(PTP_ID, ptpLPAddress, LIQUIDITY * 9999 / 10000);
        ...
        vm.prank(CHARLIE);
        vm.expectEmit(true, true, false, true);
        emit LiquidityUnstaked(CHARLIE, JOE_ID, joeLPAddress, LIQUIDITY * 99 / 200);
        // entire pool should be claimed
        policyCenter.unstakeLiquidity(JOE_ID, joeLPAddress, LIQUIDITY * 99 / 200);
    ...
    function testBuyCover() public {
        ...
        vm.prank(ALICE);
        joe.approve(address(policyCenter), type(uint256).max);
        joe.mint(ALICE, maxPayment * SCALE);
        ...
        // buy with shield
        shield.mint(CHARLIE, maxPayment * 1e12);

        vm.prank(CHARLIE);
        vm.expectRevert("ERC20: transfer amount exceeds balance");
        policyCenter.buyCover(JOE_ID, COVER_AMOUNT, 3, price);
        ...
        vm.prank(CHARLIE);
        vm.expectRevert(PriorityPool__UnderMinCoverAmount.selector);
        ...
        console.log("amount to pay", maxPayment * 10**12);
        joe.mint(CHARLIE, price * 10**12);
    ...
    function _buyJoeCover(address _user) internal {
        (uint256 price, ) = joePool.coverPrice(COVER_AMOUNT, 3);
        uint256 maxPayment = price * 101 / 100;
        vm.prank(_user);
        joe.approve(address(policyCenter), type(uint256).max);
        // cover in usd, has to convert to joe
    ...
    function testClaimPayout() public {
        ...
        ptp.mint(ALICE, price * 10**(ptp.decimals() - USDC.decimals()));
        vm.prank(ALICE);
        // Get Joe cover right address and buy cover
        ...
        joe.mint(ALICE, price * 1e12);

        vm.prank(ALICE);
        policyCenter.buyCover(JOE_ID, COVER_AMOUNT, 3, price * 11 / 10);
